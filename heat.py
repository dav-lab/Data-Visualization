...
source = ColumnDataSource(
    data=dict(weekday=weekday, day=day, month=month, color=color, number=numberEmails, event=events)
)
output_file('emails.html')
TOOLS = "resize,hover,save,pan,box_zoom,wheel_zoom"
p = figure(title="CS111 Emails",
    x_range=weekdays, y_range=list(reversed(months)),
    x_axis_location="above", plot_width=900, plot_height=400,
    toolbar_location="left", tools=TOOLS)

p.rect("weekday", "month", 1, 1, source=source,
